window.addEventListener("DOMContentLoaded", event => {
	AsuFooter.initASUFooter({
		targetSelector: "#asu-react-footer",
		props: {
			social: {
				unitLogo: "https://engineering.asu.edu/wp-content/themes/asu-divi/src/img/endorsed-logo/asu_fultonengineering_white.png",
				mediaLinks: {
					facebook: "https://facebook.com",
					twitter: "https://twitter.com/?lang=en",
					instagram: "https://instagram.com",
					linkedIn: "https://www.linkedin.com/",
					youtube: "https://www.youtube.com/",
				},
			},
			contact: {
				title: "Complete Name of College, School or Unit Title Should Go Here",
				contactLink: "#",
				contributionLink: "#",
				columns: [
					{
						title: "Column Two",
						links: [
							{
								url: "#",
								title: "link",
								text: "Biological and Health Systems Computing",
							},
							{
								url: "#",
								title: "link",
								text: "Informatics and Decision Systems Electrical",
							},
							{
								url: "#",
								title: "link",
								text: "Computer and Energy Matter",
							},
							{
								url: "#",
								title: "link",
								text: "Transport and Energy Sustainability and the Built Environment",
							},
							{
								url: "#",
								title: "link",
								text: "The Polytechnic School",
							},
						],
					},
					{
						title: "Column Three",
						links: [
							{
								url: "#",
								title: "link",
								text: "Biological and Health Systems Computing",
							},
							{
								url: "#",
								title: "link",
								text: "Transport and Energy Sustainability and the Built Environment",
							},
							{
								url: "#",
								title: "link",
								text: "The Polytechnic School",
							},
						],
					},
					{
						title: "The Zen Master and the Hot Dog Vendor",
						links: [
							{
								url: "#",
								title: "link",
								text: "A Zen master",
							},
							{
								url: "#",
								title: "link",
								text: "Visiting New York",
							},
							{
								url: "#",
								title: "link",
								text: "Gets hungry from his travels",
							},
							{
								url: "#",
								title: "link",
								text: "And asks a hot dog vendor",
							},
							{
								url: "#",
								title: "link",
								text: "To make him one",
							},
							{
								url: "#",
								title: "link",
								text: "With everything",
							}
						],
					},
					{
						title: "Column Two",
						links: [
							{
								url: "#",
								title: "link",
								text: "Biological and Health Systems Computing",
							},
							{
								url: "#",
								title: "link",
								text: "Informatics and Decision Systems Electrical",
							},
							{
								url: "#",
								title: "link",
								text: "Computer and Energy Matter",
							},
							{
								url: "#",
								title: "link",
								text: "Transport and Energy Sustainability and the Built Environment",
							},
							{
								url: "#",
								title: "link",
								text: "The Polytechnic School",
							},
						],
					},
					{
						title: "Column Three",
						links: [
							{
								url: "#",
								title: "link",
								text: "Biological and Health Systems Computing",
							},
							{
								url: "#",
								title: "link",
								text: "Transport and Energy Sustainability and the Built Environment",
							},
							{
								url: "#",
								title: "link",
								text: "The Polytechnic School",
							},
						],
					},
					{
						title: "The Zen Master and the Hot Dog Vendor",
						links: [
							{
								url: "#",
								title: "link",
								text: "A Zen master",
							},
							{
								url: "#",
								title: "link",
								text: "Visiting New York",
							},
							{
								url: "#",
								title: "link",
								text: "Gets hungry from his travels",
							},
							{
								url: "#",
								title: "link",
								text: "And asks a hot dog vendor",
							},
							{
								url: "#",
								title: "link",
								text: "To make him one",
							},
							{
								url: "#",
								title: "link",
								text: "With everything",
							}
						],
					},

				],
			},
		},
	});
});
